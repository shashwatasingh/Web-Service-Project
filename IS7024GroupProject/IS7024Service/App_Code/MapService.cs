...
public class MapService : System.Web.Services.WebService
{
    ...
    public XmlDocument AddressAutoComplete(string Address)
    {
        ...
        HttpWebRequest request = CreateWebRequest(@"https://maps.googleapis.com/maps/api/place/autocomplete/xml?key=AIzaSyA13uOGsOWCMm8K1_jDm03hKMgVscSliTg&input=" + Address);
    ...
    public XmlDocument PlaceDetails(string PlaceID)
    {
        ...
        HttpWebRequest request = CreateWebRequest(@"https://maps.googleapis.com/maps/api/place/details/xml?key=AIzaSyA13uOGsOWCMm8K1_jDm03hKMgVscSliTg&placeid=" + PlaceID);
        using (WebResponse response = request.GetResponse())
        {
            using (StreamReader rd = new StreamReader(response.GetResponseStream()))
            {
