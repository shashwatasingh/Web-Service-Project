...
public partial class _Default : System.Web.UI.Page
{
    ...
    protected void btnClientLoad_Click(object sender, EventArgs e)
    {
        ...
        {
            string[] ClientData = Clients[index].Split('|');
            string address = ClientData[4] + ", " + ClientData[5];
            string fullname = ClientData[3] + " " + ClientData[1] + " " + ClientData[2];
            string id = ClientData[0];
            decimal latitude = 0;
            decimal longitude = 0;
    ...
    protected void btnFieldRepLoad_Click(object sender, EventArgs e)
    {
        ...
        dynamic FieldRepData = JObject.Parse(restResult);
        lbFieldReps.Items.Clear();
        foreach (var Person in FieldRepData.People.Person)
        {
    ...
    protected void lbClients_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            if (xeWeather != null)
            {
                ...
                tbClientHumidity.Text = humidity;
            }
        }
        catch (Exception ex)
        { }
    ...
    private void FillInFieldRepMarkers()
    {
        ...
        MySelectedFieldRepMarkerPosition = "[]";
        MyFieldRepMarkerPositions = "";
        foreach (ListItem fieldRep in lbFieldReps.Items)
        {
            string[] value = fieldRep.Value.Split('|');
            if (fieldRep.Selected == true)
            {
